...
struct SeriesDetailView: View {
    ...
    @EnvironmentObject var store: AppStore
    @State private var seasons: [(season: Int, episodes: [Episode])] = []

    var body: some View {
        List {
            ForEach(seasons, id: \.season) { s in
                Section(header: Text("Sezoni \(s.season)")) {
                    ForEach(s.episodes) { ep in
                        NavigationLink {
                            PlayerView(kind: .episode(ep, title: "\(series.name) — S\(s.season) E\(ep.num)"))
                        } label: {
                            Text("Episodi \(ep.num)")
    ...
    @MainActor private func loadEpisodes() async {
        ...
        if let info: SeriesInfoResponse = try? await api.get(
            ...
            seasons = eps.keys.compactMap { Int($0) }
                .sorted()
                .map { ($0, (eps[String($0)] ?? []).sorted { $0.num < $1.num }) }
        }
    }
}
